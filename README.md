...
```python
k - (len(big) + len(equal))如何理解？
在快速选择算法中，当确认目标数字落在了 `small`（小数）桶里时，
意味着 `big`（大数）桶和 `equal`（相等数）桶里的所有数字，都比我们要找的目标数字**更大**。
既然它们更大，它们就理所当然地占据了排名最靠前的名次。当我们把搜索范围缩小到仅仅只看 `small` 桶时，
因为前面那些较大的数字被我们“丢弃”了，所以目标的**相对名次**发生了变化，必须把前面已经被占用的名次坑位给减掉。
### 通俗的比喻：考试排名
假设年级里要找**第 5 名**的学生（$k=5$）。
已知：
1.  一班有 3 个学生，他们考得分数全都比二班的学生高（相当于 `big` 和 `equal` 桶，总人数为 3）。
2.  既然前 3 名全被一班包揽了，那么年级**第 5 名**绝对不可能在一班，只能去二班（`small` 桶）找。
3.  那么在二班内部，还要找第 5 名吗？显然不是。因为年级前 3 名已经在一班确定了，所以年级第 5 名，实际上就是二班内部的**第 2 名**（$5 - 3 = 2$）。
这里的 $5$ 就是 $k$，$3$ 就是 `len(big) + len(equal)`，$2$ 就是 `k - (len(big) + len(equal))`。
```
  * `return pivot`
